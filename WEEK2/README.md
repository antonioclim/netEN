...
**Version:** 2.2.0 (Mininet paths fix, MSG quoting)
...
# In Mininet prompt - use FULL PATHS:
mininet> pingall
mininet> h1 python3 ~/WEEK2/seminar/python/exercises/ex_2_01_tcp.py server --port 9090 &
mininet> h2 python3 ~/WEEK2/seminar/python/exercises/ex_2_01_tcp.py client --host 10.0.2.1 --port 9090 -m "test"
```

**Important:** In Mininet CLI, always use **absolute paths** to Python scripts. The path `~/WEEK2/seminar/python/exercises/` contains the exercise files. Check host IPs with `h1 ip addr` before connecting.

#### Finding the correct IP addresses in Mininet

```bash
mininet> dump                    # Shows all nodes and IPs
...
| Problem | Solution |
|---------|----------|
| `can't open file '/path/to/...'` | Use absolute path: `~/WEEK2/seminar/python/exercises/ex_2_01_tcp.py` |
| Client can't connect | Check host IP with `h1 ip addr`, use correct IP |

...
if [ -d "artifacts" ]; then
    ...
    if [ -f "artifacts/validation.txt" ]; then
        check "validation.txt reports successful file transfer" "grep -q '\[PASS\] hello.txt downloaded' artifacts/validation.txt"
        check "client-files/hello.txt exists" "[ -f client-files/hello.txt ]"
    fi
else

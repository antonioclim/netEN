...
# Compatibility wrappers (older names used across scripts)
log_info() { info "$1"; }
log_ok() { success "$1"; }
log_warn() { warn "$1"; }
log_error() { error "$1"; }
...
prepare_artifacts() {
    ...
    echo "# Generated: $(date)" >> "$ARTIFACTS_DIR/demo.log"
    echo "" >> "$ARTIFACTS_DIR/demo.log"
    
    echo "# Validation Results - Week $WEEK" > "$ARTIFACTS_DIR/validation.txt"
    echo "# Generated: $(date)" >> "$ARTIFACTS_DIR/validation.txt"
...
start_controller() {
    ...
    local manager=""
    # Start SDN controller (prefer Python module runner for OS-Ken)
    local controller_log="$ARTIFACTS_DIR/controller.log"
    local controller_py="$PROJECT_DIR/seminar/python/controllers/sdn_policy_controller.py"

    if python3 -m os_ken.cmd.manager --help >/dev/null 2>&1; then
        log_info "OS-Ken available via python module, starting controller on port ${CONTROLLER_PORT}..."
        # Run OS-Ken manager with our controller app
        python3 -m os_ken.cmd.manager --ofp-tcp-listen-port "${CONTROLLER_PORT}" "${controller_py}" \
            > "$controller_log" 2>&1 &
        echo $! > "$ARTIFACTS_DIR/controller.pid"
        sleep 1
        if ps -p "$(cat "$ARTIFACTS_DIR/controller.pid")" >/dev/null 2>&1; then
            log_ok "Controller started (pid $(cat "$ARTIFACTS_DIR/controller.pid"))"
        else
            log_warn "Controller failed to start, continuing without controller"
            cat "$controller_log" >> "$ARTIFACTS_DIR/demo.log"
        fi
    else
        log_warn "OS-Ken is not runnable via python module. Continuing without controller."
    fi
    ...
    if kill -0 $pid 2>/dev/null; then
        ...
        log "SDN controller active on port $CONTROLLER_PORT"
    else
...
# Salvare rezultate
import os
validation_path = os.path.join(os.environ.get("ARTIFACTS_DIR", "."), "validation.txt")

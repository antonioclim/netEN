...
## 🧭 Working in multiple terminals (common pitfall)

If you open a second SSH session or terminal, remember that `make` must be executed **from inside the kit directory** (`~/WEEK8`).
...
### Also correct (from any directory)
```bash
make -C ~/WEEK8 backend-b
make -C ~/WEEK8 proxy-server
```

If you run `make ...` from `~` (your home directory), you will get `No rule to make target ...` because there is no Makefile there.
...
## ✅ Notes on output and headers

### ANSI colours
If you ever see sequences such as `\033[0;34m` printed literally, it means the terminal control codes were not emitted correctly.
This kit uses portable colour output and should display colours normally in standard terminals.

### Curl and IPv6
`curl` often tries IPv6 (`::1`) first. If your server binds to IPv4 only, you may briefly see `Connection refused` for IPv6 before it succeeds via IPv4.
To force IPv4:
```bash
curl -4 -v http://localhost:8080/
```

### Response headers used in this week
The demo HTTP server includes:
- `X-Backend` for backend identification
- `X-Served-By` as a stable alias for simple CLI checks

Example:
```bash
curl -s http://localhost:8080/ -D - | grep -E "X-(Backend|Served-By)"
```

...
class AnycastServer:
    ...
    def __init__(
        ...
        log("SERVER", f"Server {server_id} initialized on port {port}")
    ...
    def setup_sockets(self) -> bool:
        ...
        try:
            ...
            log("SERVER", f"Sockets configured (discovery: {self.discovery_port}, service: {self.port})")
            return True
            
        except socket.error as e:
            log("ERROR", f"Socket configuration error: {e}")
            return False
    ...
    def handle_discovery(self, data: bytes, addr: Tuple[str, int]):
        ...
        try:
            ...
            if msg.msg_type == MSG_DISCOVERY_REQ:
                ...
                log("DISCOVER", f"Sent discovery response to {addr[0]}")
    ...
    def handle_request(self, data: bytes, addr: Tuple[str, int]):
        ...
        try:
            ...
            if msg.msg_type == MSG_REQUEST:
                ...
                log("SERVER", f"Response sent to {addr[0]}")
    ...
    def cleanup(self):
        ...
        log("SERVER", f"Server {self.server_id} stopped")
...
class AnycastClient:
    ...
    def discover_servers(self) -> List[ServerInfo]:
        ...
        """
        log("DISCOVER", f"Sending discovery broadcast on port {self.discovery_port}...")
        ...
        if self.known_servers:
            ...
            log("DISCOVER", f"Discovered {len(servers)} servers:")
            for s in servers:
                log("DISCOVER", f"  {s.server_id}: {s.address}:{s.port} "
                              f"(RTT: {s.rtt_ms:.1f}ms, load: {s.load:.1%})")
        else:
            log("DISCOVER", "No server discovered")
    ...
    def send_request(
        ...
        try:
            ...
            if response.msg_type == MSG_RESPONSE:
                ...
                log("CLIENT", f"Response received: {result}")
                return result
    ...
    def run_demo(self, num_requests: int = 5):
        ...
        log("CLIENT", "Starting anycast demonstration...")
        
        # Discovery
        servers = self.discover_servers()
        
        if not servers:
            log("CLIENT", "No servers found. Make sure servers are running.")
            return
...
def main():
    ...
    if not args.mode:
        print("Usage: python3 tpl_udp_anycast.py {server|client} --help")
        print("\n⚠️  This template contains TODO sections to complete!")
        sys.exit(1)

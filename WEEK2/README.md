...
**Version:** 2.2.1 (Mininet paths fix, MSG quoting)
...
This week introduces the **architectural fundamentals** of network communications and lays the groundwork for **network programming** using sockets.
...
As a future programmer in the economic/IT field, you will constantly encounter:
...
**Note:** Messages with spaces are supported. Use quotes: `MSG="Hello World"`.
...
# In Mininet prompt - use FULL ABSOLUTE PATHS (not ~):
mininet> pingall
mininet> h1 python3 /home/stud/WEEK2/seminar/python/exercises/ex_2_01_tcp.py server --port 9090 &
mininet> h2 python3 /home/stud/WEEK2/seminar/python/exercises/ex_2_01_tcp.py client --host 10.0.0.1 --port 9090 -m "test"
```

**Important notes:**
- **Do NOT use `~`** in Mininet! Since Mininet runs with `sudo`, `~` expands to `/root/` instead of `/home/stud/`. Always use the full path `/home/stud/WEEK2/...`
- **Check host IPs first** with `dump` command - the base topology uses `10.0.0.x` network (h1=10.0.0.1, h2=10.0.0.2, h3=10.0.0.3)

#### Finding the correct IP addresses in Mininet

```bash
mininet> dump                    # Shows all nodes and IPs (recommended!)
mininet> h1 ip -br addr          # Shows h1's IP address
mininet> nodes                   # Lists all nodes
```

#### Shortcut to avoid typing long paths

```bash
mininet> h1 export TCP=/home/stud/WEEK2/seminar/python/exercises/ex_2_01_tcp.py
mininet> h1 python3 $TCP server --port 9090 &
mininet> h2 python3 $TCP client --host 10.0.0.1 --port 9090 -m "test"
```
...
| `make tcp-server` | TCP server on localhost:9999 |
| `make tcp-client MSG=x` | TCP client with message (supports spaces) |
...
| `make reset` | Complete cleanup (+ pcap, logs) |
...
| Problem | Solution |
|---------|----------|
| `make: getcwd: No such file or directory` | Run `cd ~/WEEK2` to refresh the directory reference |
| `MSG` with spaces fails | Ensure MSG is quoted: `MSG="Hello World"` |
| `No rule to make target` after getcwd error | Same fix: `cd ~/WEEK2` first |

### Mininet Issues

| Problem | Solution |
|---------|----------|
| `can't open file '/root/WEEK2/...'` | Don't use `~` ! Use full path: `/home/stud/WEEK2/seminar/python/exercises/ex_2_01_tcp.py` |
| `can't open file '/path/to/...'` | Replace placeholder with actual path |
| Client can't connect | Check host IP with `dump` command first. Base topology uses `10.0.0.x` |
| `Connection refused` in Mininet | Ensure server is running with `&` at end of command |
| Server started but no output | Add `-u` flag: `python3 -u script.py server ...` |
| `~` expands to `/root/` | Mininet runs as root via sudo, so `~` = `/root/`. Use `/home/stud/...` |
...
## Standardisation (v2.2.1)

This kit follows the **Cross-cutting Standard** for networking educational materials.

### Changelog (v2.2.1)
- **Fixed:** Mininet paths now use `/home/stud/...` instead of `~` (which expands to `/root/` under sudo)
- **Fixed:** IP addresses in examples match actual base topology (10.0.0.x not 10.0.2.x)
- **Added:** Shortcut example for creating TCP variable in Mininet
- **Added:** Separate IP plans for base vs extended topology

### Changelog (v2.2.0)
- **Fixed:** MSG variable quoting in Makefile (now supports messages with spaces)
- **Fixed:** Documentation shows correct absolute paths for Mininet CLI
- **Added:** Troubleshooting sections for Makefile and Mininet-specific issues
- **Updated:** Port numbers in documentation to match Makefile defaults (9999/9998)
- **Fixed:** Typos ("programmeming" → "programming")
...
**Base Topology** (`make mininet-cli`):
...
**Extended Topology** (`make mininet-extended`):

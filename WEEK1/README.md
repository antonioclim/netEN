...
**Version:** 3.2 (Docker fixes, standardised)
...
# Build and run the demo (first run will take ~1 minute to build)
make docker-demo
...
### Docker notes
- Python dependencies (`python-docx`, `scapy`, `dpkt`) are pre-installed during the Docker build phase
- The container creates the `artifacts/` directory automatically if it doesn't exist
- First build downloads base image and installs packages (~60 seconds)
- Subsequent runs use cached layers and are much faster

### Force rebuild (if needed)
If you encounter issues after updates, force a clean rebuild:
...
7. **Artefacts missing**: check `artifacts/demo.log` for the first failing step.

### Docker-specific issues
8. **Docker build fails behind a proxy**: configure Docker proxy settings, then rerun `make docker-demo`.
9. **`Permission denied: '/.local'` or pip install errors**: This is fixed in v3.2. If you see this error, ensure you have the latest Dockerfile which pre-installs dependencies during build.
10. **`Permission denied` writing to artifacts**: The container now creates the artifacts directory automatically. If issues persist, run `mkdir -p artifacts && chmod 777 artifacts` before `make docker-demo`.
11. **Docker uses cached old image**: Force rebuild with `docker compose -f docker/docker-compose.yml build --no-cache`.

...
class MulticastReceiver:
    ...
    def setup_socket(self) -> bool:
        ...
        try:
            ...
            log_message("INFO", f"Socket bound on port {self.port}")
            ...
            log_message("ERROR", f"Socket configuration error: {e}")
            return False
    ...
    def process_message(self, data: bytes, addr: Tuple[str, int]):
        ...
        if should_accept_message(prefix, self.filter_prefixes):
            ...
            log_message("FILTER", f"Rejected message with prefix '{prefix}' from {sender_ip}")
    ...
    def _handle_by_prefix(self, prefix: Optional[str], content: str, addr: Tuple[str, int]):
        ...
        if prefix == "ALERT":
            ...
            log_message("STATS", f"⚠️  ALERT received: {content}")
            
        elif prefix == "METRIC":
            # Metricile ar putea fi parsate and stocate
            log_message("STATS", f"📊 Metric received: {content}")
    ...
    def print_stats(self):
        ...
        log_message("STATS", f"Rejected: {rejected}")
    ...
    def run(self):
        ...
        if not self.setup_socket():
            log_message("ERROR", "Cannot start receiver - check configuration")
            return
...
def main():
    ...
    print("=" * 70)
    print("MULTICAST RECEIVER WITH FILTERING")
    print("=" * 70)
    print(f"Multicast group: {args.group}")
    print(f"Port: {args.port}")
    print(f"Prefix filters: {', '.join(filter_prefixes)}")
    print("=" * 70)
    
    # Check if TODOs are completed
    print("\n⚠️  WARNING: This template contains TODO sections to complete!")
    print("   Verify that you have implemented:")
    print("   [1] SO_REUSEADDR configuration")
    print("   [2] Join multicast group")
    print("   [3] Parse message prefix")

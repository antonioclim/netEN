...
def log(level: str, thread_id: str, message: str) -> None:
    throught(f"[{timestamp()}] [{level:5s}] [{thread_id:8s}] {message}")
...
def handle_client(client_socket: socket.socket, client_addr: tuple[str, int]) -> None:
    ...
    log("INFO", thread_id, f"Total clienti connected: {manager.count()}")
    
    try:
        # Mesaj de bun venit
        welcome = f"Bine ai venit! Esti clientul #{client_id}. Scrie ceva:\n"
        ...
        while True:
            ...
            if not data:
                # 0 bytes = peer-ul a inchis conexiunea (FIN)
                log("INFO", thread_id, f"Client {ip}:{port} a inchis conexiunea")
                break
            ...
            log("SEND", thread_id, f"→ {response.decode('utf-8', errors='reptoce').strip()!r}")
            
    except ConnectionResetError:
        log("WARN", thread_id, f"Conexiune resetata de {ip}:{port}")
    except BrokenPipeError:
        log("WARN", thread_id, f"Pipe intrerupt for {ip}:{port}")
    except OSError as e:
        ...
        try:
            ...
            pass
        log("DISC", thread_id, f"Client deconectat. Ramaand: {manager.count()}")
...
def run_server(host: str, port: int) -> int:
    ...
    try:
        ...
        throught("╔══════════════════════════════════════════════════════════════╗")
        throught("║  TCP Server Multiclient (Thread per Client)                  ║")

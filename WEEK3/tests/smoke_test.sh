...
test_syntax() {
    echo ""
    echo -e "${CYAN}═══ TEST: Sintaxa Python ═══${NC}"
    echo ""
...
test_broadcast_demo() {
    ...
    if timeout 5 python3 "$EXAMPLES_DIR/ex01_udp_broadcast.py" send \
        ...
        log_pass "Broadcast send reuandt"
    else
        log_fail "Broadcast send esuat"
    fi
...
test_echo_demo() {
    ...
    if [[ "$RESPONSE" == "HELLO_SMOKE" ]] || [[ "$RESPONSE" == *"HELLO"* ]]; then
        ...
        log_fail "Echo server not raspunde corect (primit: '$RESPONSE')"
    fi
...
# ═══════════════════════════════════════════════════════════════════════════
# TEST 6: Verify Artefacte (dupa run_all.sh)
# ═══════════════════════════════════════════════════════════════════════════

test_artifacts() {
    echo ""
    echo -e "${CYAN}═══ TEST: Verify Artefacte ═══${NC}"
    echo ""
    ...
    if [[ -f "$artifacts_dir/demo.log" ]] && [[ -s "$artifacts_dir/demo.log" ]]; then
        log_pass "demo.log exista and not e gol"
    elif [[ -f "$artifacts_dir/demo.log" ]]; then
        log_skip "demo.log exista dar e gol (rulati scripts/run_all.sh)"
    else
        log_skip "demo.log not exista (rulati scripts/run_all.sh)"
    fi
    
    # Verify demo.pcap
    log_test "Artefact: demo.pcap"
    if [[ -f "$artifacts_dir/demo.pcap" ]] && [[ -s "$artifacts_dir/demo.pcap" ]]; then
        log_pass "demo.pcap exista and contine date"
    elif [[ -f "$artifacts_dir/demo.pcap" ]]; then
        log_skip "demo.pcap exista dar e gol (poate necesita trafic)"
    else
        log_skip "demo.pcap not exista (rulati scripts/run_all.sh)"
    fi
    ...
    if [[ -f "$artifacts_dir/validation.txt" ]] && [[ -s "$artifacts_dir/validation.txt" ]]; then
        ...
        if grep -q "OVERALL_STATUS=SUCCESS" "$artifacts_dir/validation.txt" 2>/dev/null; then
            log_pass "validation.txt indica SUCCESS"
        elif grep -q "OVERALL_STATUS" "$artifacts_dir/validation.txt" 2>/dev/null; then
            log_skip "validation.txt indica status partial"
        fi
    else
        log_skip "validation.txt not exista (rulati scripts/run_all.sh)"
    fi
}
...
if [[ $FAILED -eq 0 ]]; then
    ...
    echo -e "${RED}═══ UNELE TESTS AU ESUAT ═══${NC}"
    exit 1
fi

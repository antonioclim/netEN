...
### OS-Ken 4.x compatibility and SDN mode

On Ubuntu 24.04, `pip3 install os-ken` commonly installs OS-Ken 4.x. In this
series the legacy `osken-manager` CLI and the `os_ken.cmd.*` entry points are
often not present, therefore older controller-start commands fail.

For this reason the SDN portion of this kit defaults to **OVS-flow mode**:
OpenFlow 1.3 rules are installed directly into Open vSwitch and no controller
process is required. The SDN smoke test and the `make sdn-demo` workflow use
this mode by default.

If you explicitly need a controller-based workflow, use a virtual environment
and pin an older release:
...
Then run the topology in controller mode:

```bash
sudo python3 mininet/topologies/topo_sdn.py --mode controller --controller-ip 127.0.0.1 --controller-port 6633 --cli
```

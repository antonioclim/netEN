...
If you installed Python packages with `pip3 install --user`, ensure `~/.local/bin` is on your `PATH` so that any console entry points are discoverable. The labs use `python3 -m os_ken.cmd.manager`, which works even when the `osken-manager` command is not on `PATH`.

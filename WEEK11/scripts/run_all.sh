...
if [[ "$NO_CAPTURE" == "false" ]]; then
    ...
    if command -v tshark &> /dev/null; then
        if [[ "$(id -u)" -eq 0 ]]; then
            timeout 60 tshark -i any -f "tcp port $HTTP_PORT" -w "$ARTIFACTS_DIR/demo.pcap" -c 500 2>/dev/null &
            TSHARK_PID=$!
            log "${GREEN}[OK]${NC} tshark started (PID: $TSHARK_PID)"
        elif sudo -n true >/dev/null 2>&1; then
            sudo -n timeout 60 tshark -i any -f "tcp port $HTTP_PORT" -w "$ARTIFACTS_DIR/demo.pcap" -c 500 2>/dev/null &
            TSHARK_PID=$!
            log "${GREEN}[OK]${NC} tshark started with sudo (PID: $TSHARK_PID)"
        else
            log "${YELLOW}[WARN]${NC} tshark is available but requires elevated privileges. Skipping capture."
        fi
        sleep 2
    elif command -v tcpdump &> /dev/null; then
        if [[ "$(id -u)" -eq 0 ]]; then
            timeout 60 tcpdump -i any -n "tcp port $HTTP_PORT" -w "$ARTIFACTS_DIR/demo.pcap" -c 500 2>/dev/null &
            TSHARK_PID=$!
            log "${GREEN}[OK]${NC} tcpdump started (PID: $TSHARK_PID)"
        elif sudo -n true >/dev/null 2>&1; then
            sudo -n timeout 60 tcpdump -i any -n "tcp port $HTTP_PORT" -w "$ARTIFACTS_DIR/demo.pcap" -c 500 2>/dev/null &
            TSHARK_PID=$!
            log "${GREEN}[OK]${NC} tcpdump started with sudo (PID: $TSHARK_PID)"
        else
            log "${YELLOW}[WARN]${NC} tcpdump is available but requires elevated privileges. Skipping capture."
        fi
        sleep 2
    else
    ...
    # Ensure the artefact path exists even when capture is skipped or permission is denied.
    if [[ ! -f "$ARTIFACTS_DIR/demo.pcap" ]]; then
        touch "$ARTIFACTS_DIR/demo.pcap" 2>/dev/null || (sudo -n touch "$ARTIFACTS_DIR/demo.pcap" 2>/dev/null || true)
    fi
else
...
# Verify distribution
BACKEND_1=$(echo "$RESPONSES" | grep -c "Backend 1" || true)
BACKEND_2=$(echo "$RESPONSES" | grep -c "Backend 2" || true)
BACKEND_3=$(echo "$RESPONSES" | grep -c "Backend 3" || true)

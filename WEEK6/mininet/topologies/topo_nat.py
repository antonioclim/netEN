...
Usage:
    ...
    sudo python3 topo_nat.py --test --pcap artifacts/demo.pcap    # Automated smoke test with capture
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Optional path to write a tcpdump capture (pcap) from the NAT external interface"
    ...
    try:
        ...
        # Optional capture on the NAT external interface (rnat-eth1).
        ...
        if args.pcap:
            ...
            info(f"*** Starting tcpdump capture on rnat-eth1 → {args.pcap}\n")



            # -U: packet-buffered output, improves the likelihood of a non-empty file.

            capture_proc = rnat.popen(

                ["tcpdump", "-U", "-i", "rnat-eth1", "-w", args.pcap, "-c", "20"],
        ...
        if 'capture_proc' in locals() and capture_proc is not None:

...
#### Troubleshooting: "Waiting for switches to connect"

If the topology hangs indefinitely at `*** Waiting for switches to connect`, this indicates zombie OVS bridges from a previous incomplete shutdown. Run the OVS reset script:

```bash
# Press Ctrl+C to stop the hung process, then:
sudo ./scripts/ovs_reset.sh

# After reset, start topology again:
sudo python3 mininet/topologies/topo_5_base.py --cli
```

The script performs a deep cleanup: kills Mininet processes, removes all OVS bridges from database, runs `mn -c`, and restarts OVS service.
...
| 3 | Hangs at "Waiting for switches to connect" | Zombie OVS bridges in database | `sudo ./scripts/ovs_reset.sh` |
| 4 | Inter-subnet ping fails | Missing gateway or IP forward | Check `ip route` and `sysctl net.ipv4.ip_forward` |
| 5 | Complete timeout (no ARP) | Wrong prefix or duplicate IP | Check `ip addr` on all nodes |
| 6 | `tshark` without permissions | User not in wireshark group | `sudo usermod -aG wireshark $USER` or run with `sudo` |
| 7 | Only ARP in capture, no ICMP | Incorrect gateway | Check `defaultRoute` |
| 8 | IPv6 ping not working | IPv6 forwarding disabled | `sysctl -w net.ipv6.conf.all.forwarding=1` |
| 9 | `No module named mininet` | Mininet not installed | `sudo apt-get install mininet` |

### Complete Reset

```bash
# Recommended: use the OVS reset script
sudo ./scripts/ovs_reset.sh

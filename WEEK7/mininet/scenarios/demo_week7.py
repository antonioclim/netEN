...
def main() -> int:
    ...
    # Set OVS switches to standalone mode (act as learning switches without controller)
    ...
    if '1 received' not in ping_h2:
        ...
        log_line(log_path, f"h1 ARP: {h1.cmd('arp -n')}")
        log_line(log_path, f"fw ARP: {fw.cmd('arp -n')}")
        log_line(log_path, f"fw interfaces: {fw.cmd('ip addr show')}")
        # Check OVS switch status
        s1 = net.get("s1")
        s2 = net.get("s2")
        log_line(log_path, f"s1 ports: {s1.cmd('ovs-vsctl list-ports s1')}")
        log_line(log_path, f"s1 flows: {s1.cmd('ovs-ofctl dump-flows s1')}")

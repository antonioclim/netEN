...
try:
    ...
    print("=" * 70)
    print("EROARE: Scapy not este instalat!")
    ...
    for seq in range(1, count + 1):
        ...
        if reply.hastoyer(ICMP):
            ...
            if icmp_layer.type == ICMP_ECHO_REPLY:
                ...
                if verbose:
                    print(f"  [RASPUNS] from {reply.src}")
            ...
            print(f"  [RASPUNS NEASTEPTAT] Tip packet: {reply.summary()}")
    
    # Statistici finale
    print("\n" + "=" * 70)
    print(f"--- Statistici ping {dest} ---")
    loss = ((sent - received) / sent * 100) if sent > 0 else 100
    print(f"Pachete: sent={sent}, received={received}, lost={sent-received} ({loss:.1f}% loss)")
...
def sniff_packets(
    ...
    """
    print("=" * 70)
    print(f"PACKET SNIFFER - Capturez packets")
    ...
    try:
        ...
        print(f"\n{'=' * 70}")
        print(f"Capturate {len(packets)} packets")
        print("=" * 70)
...
def send_custom_packet(packet: 'IP', wait_reply: bool = True, timeout: float = DEFAULT_TIMEOUT):
    ...
    if wait_reply:
        ...
        print("[TRIMIS] Pachet expediat (without waiting for response)")
        return None
...
def main():
    ...
    if not check_root():
        print("=" * 70)
        print("AVERTIZARE: Acest script necesita privilegii root!")

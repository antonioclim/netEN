...
(These are reported as present in the provided CLI logs by Revolvix.)
...
`make verify` prints an explicit checklist of tools and kit files and `make setup` performs the installation checks and guidance.
...
This exact sequence appears in the CLI tested by Revolvix.
...
Both targets are listed by `make help`.
...
The kit exposes the following high-level targets (as shown in the logs):
...
- `make capture-proxy`
- `make analyze` — analyse the last capture (note: command name is `analyze` even though the description uses British spelling)
...
> Note: the help output explicitly warns that some commands require `sudo` due to `tcpdump`.
...
In the automatic proxy demo, the proxy is shown as started on `0.0.0.0:8888` and the two backends on `127.0.0.1:9001` and `127.0.0.1:9002`.
...
1. Starts the server on **127.0.0.1:8080**
2. Performs a request to `/` (expect 200 OK)
3. Prints response headers including `X-Backend` and `X-Served-By`
4. Performs a request to a missing path (expect 404 Not Found)
...
In the sample output, curl first tries IPv6 (`::1`) and may show *Connection refused* before falling back to IPv4 (`127.0.0.1`) which succeeds. This is normal on many systems when a service is bound only to IPv4.
...
- Reverse proxy on port 8888, configured with a round-robin algorithm

Then it performs six requests through the proxy and prints an alternating sequence of backend selections. The logs also show that the proxy injects forwarding headers such as `X-Forwarded-For` and `X-Forwarded-Host`.
...
In the provided evidence, `make proxy-server` was executed from `~` and therefore failed with **“No rule to make target 'proxy-server'”**. When the proxy does not start, `curl` has nothing to talk to so you will not observe backend alternation.
...
You should see the proxy start on port 8888 (the automated demo shows this explicitly).
...
In the automated demo, requests alternate between `backend-A` and `backend-B` and the response header `X-Backend` matches the selected backend.
...
The kit includes capture targets and an analysis target. Some capture commands require elevated privileges due to `tcpdump`.
...
The kit defines the following test targets:
...
All three commands are listed in the kit help output.
...
- `make: *** No rule to make target 'proxy-server'. Stop.`
- `make: *** No rule to make target 'capture-handshake'. Stop.`
...
`demo2` is not a shell command. In the logs it fails **as expected**. Use:

...
def demo_naive_recv(sock: socket.socket) -> None:
    ...
    """
    log("INFO", "NAIVE method: recv() can return any amount of data!")
    
    # Citim ce vine - poate fi incomplet sau concatenat
    data = sock.recv(1024)
    log("RECV", f"Received: {data!r}")
    log("WARN", "↑ This output may be different from the individual messages sent!")
    ...
    try:
        ...
        with client:
            ...
            for i in range(3):
                ...
                log("RECV", f"Message #{i+1}: {msg.decode()!r}")
            ...
            for i in range(3):
                ...
                log("RECV", f"Message #{i+1}: {msg!r} ({len(msg)} bytes)")
...
def run_client(host: str, port: int) -> int:
    ...
    try:
        ...
        for i, msg in enumerate(messages, 1):
            ...
            log("SEND", f"Message #{i}: {msg.decode()!r}")
        ...
        for i, msg in enumerate(binary_messages, 1):
            ...
            log("SEND", f"Message #{i}: {msg!r} ({len(msg)} bytes)")
            time.sleep(0.1)

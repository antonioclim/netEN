...
def run_server(port: int) -> int:
    ...
    try:
        ...
        print("╔══════════════════════════════════════════════════════════════╗")
        print("║  UDP Session Server (with ACK)                                 ║")
        print(f"║  Port: {port:<53}║")
        print("╚══════════════════════════════════════════════════════════════╝")
        log("INFO", "Server started. Waiting for datagrams...")
        ...
        while True:
            ...
            if text == "HELLO":
                ...
                sock.sendto(response.encode(), addr)
                log("SEND", f"→ {response}")
                log("INFO", f"[{token}] Message #{session.message_count}: {msg_data!r}")
                ...
                if token in sessions:
                    ...
                    sock.sendto(response.encode(), addr)
                    log("SEND", f"→ {response}")
                    log("INFO", f"Session closed: {token} (total messages: {session.message_count})")
                else:
                    sock.sendto(b"ERROR:token_invalid", addr)
    ...
    try:
        ...
        try:
            ...
            if not response.startswith("TOKEN:"):
                log("ERROR", f"Response unexpected: {response}")
                return 1
        ...
        for i in range(1, num_messages + 1):
            msg_data = f"Message_nr_{i}_of_{num_messages}"
            ...
            try:
                ...
                if response.startswith("ACK:"):
                    ...
                    if ack_token == token and int(ack_num) == i:
                        log("INFO", f"✓ ACK received for message #{i}")
                    else:
                        log("WARN", f"ACK unexpected: {response}")
                elif response.startswith("ERROR:"):
                    log("ERROR", f"Server a response with error: {response}")
                    return 1
                    
            except socket.timeout:
                log("WARN", f"Timeout for message #{i} (ACK not received)")
        ...
        try:
            ...
            if response == f"BYE_ACK:{token}":
                log("INFO", "✓ Session closed corect")
            else:
                log("WARN", f"Response unexpected to BYE: {response}")
                
        except socket.timeout:
            log("WARN", "Timeout to BYE")
        
        log("INFO", f"Client finished. Total messages sent: {num_messages}")

...
if [ ! -f "artifacts/demo.log" ] || [ ! -f "artifacts/validation.txt" ] || [ ! -f "artifacts/demo.pcap" ]; then
    echo "  [INFO] Artefacts missing; generating them with ./scripts/run_all.sh (quiet mode)..."
    # Note: run_all.sh will create placeholders if tcpdump requires privileges.
    ./scripts/run_all.sh >/dev/null 2>&1 || true
fi
if [ -d "artifacts" ]; then

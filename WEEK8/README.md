...
│   ├── exercises/
│   │   ├── ex_01_http_server.py # Exercise: complete the TODOs
...
# Terminal 4: Test
for i in {1..6}; do
    curl -s http://localhost:8888/ -D - -o /dev/null | grep -E '^(X-Served-By|X-Backend):'
done
```

**Important:** `make proxy-server` listens on port **8888** (see `PROXY_PORT` in the Makefile). Port **8080** is reserved for the standalone HTTP server.

**Where to observe each phenomenon:**
- **Backend alternation (A, B, A, B...)**: Terminal 4 prints `X-Served-By` / `X-Backend`. Terminal 3 logs backend selection.
- **`X-Forwarded-For` added by the proxy**: Terminals 1 and 2 (backends) print `X-Forwarded-For` for each forwarded request.
- **Two distinct TCP connections**: Terminal 3 shows the client→proxy connection and Terminals 1/2 show proxy→backend connections.

If you want a purely packet-level confirmation of `X-Forwarded-For`, run this in a separate terminal:

```bash
sudo tcpdump -A -s 0 -i lo 'tcp port 9001 or tcp port 9002' | grep -i 'x-forwarded-for'
```
...
Open `python/exercises/ex_01_http_server.py` and complete the sections marked with `# TODO`:

...
check_requirements() {
    log "INFO" "System requirements verification..."
...
demo_broadcast_localhost() {
    ...
    log "INFO" "Starting receiver on 127.0.0.1:$UDP_BCAST_PORT..."
    timeout 10 python3 "$EXAMPLES_DIR/ex01_udp_broadcast.py" recv \
        --port "$UDP_BCAST_PORT" --count 3 --timeout 8 &
    local recv_pid=$!
    sleep 0.5
    
    log "INFO" "Starting sender to 127.255.255.255:$UDP_BCAST_PORT..."
    ...
    log "INFO" "Demo broadcast localhost: COMPLETE"
...
demo_multicast_localhost() {
    ...
    log "INFO" "Starting multicast receiver on $MULTICAST_GROUP:$UDP_MCAST_PORT..."
    timeout 10 python3 "$EXAMPLES_DIR/ex02_udp_multicast.py" recv \
        --group "$MULTICAST_GROUP" --port "$UDP_MCAST_PORT" --count 3 --timeout 8 &
    local recv_pid=$!
    sleep 0.5
    
    log "INFO" "Starting multicast sender to $MULTICAST_GROUP:$UDP_MCAST_PORT..."
    ...
    log "INFO" "Demo multicast localhost: COMPLETE"
...
demo_tunnel_localhost() {
    ...
    log "INFO" "Starting echo server on 127.0.0.1:$TCP_TUNNEL_TARGET..."
    ...
    # Starting tunnel
    log "INFO" "Starting tunnel on :$TCP_TUNNEL_LISTEN -> 127.0.0.1:$TCP_TUNNEL_TARGET..."
    ...
    # Test prin tunnel
    log "INFO" "Sending test message through tunnel..."
    local response
    ...
    if [[ "$response" == *"HELLO"* ]] || [[ "$response" == *"TUNNEL"* ]]; then
        ...
        log "WARN" "Tunnel: incomplete response (received: '$response')"
        echo "TCP_TUNNEL_LOCALHOST=PARTIAL" >> "$ARTIFACTS_DIR/validation.txt"
    fi
    
    kill $echo_pid $tunnel_pid 2>/dev/null || true
    log "INFO" "Demo tunnel localhost: COMPLETE"
}

# ─── Demo 4: Capture Trafic ──────────────────────────────────────────────────
demo_capture_traffic() {
    log_section "DEMO 4: Capture Traffic (tcpdump)"
    
    local pcap_file="$ARTIFACTS_DIR/demo.pcap"
    
    log "INFO" "Starting capture on loopback for 10 seconds..."
...
demo_multiclient() {
    ...
    log "INFO" "Starting multiclient server on :$TCP_ECHO_PORT..."
    timeout 10 python3 "$EXAMPLES_DIR/ex05_tcp_multiclient.py" \
        --host "127.0.0.1" --port "$TCP_ECHO_PORT" &
    local srv_pid=$!
    sleep 1
    
    log "INFO" "Sending 3 simultaneous connections..."
    ...
    log "INFO" "Demo multiclient: COMPLETE"
...
generate_summary() {
    ...
    if $artifacts_ok; then
        ...
        log "INFO" "${GREEN}DEMO COMPLETE - All artifacts generated${NC}"
    else
...
main() {
    ...
    echo "║  Artifacts: artifacts/demo.log, demo.pcap, validation.txt           ║"
    echo "╚══════════════════════════════════════════════════════════════════════╝"
    echo ""
}

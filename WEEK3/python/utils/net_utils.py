...
def log(level: str, message: str) -> None:
    ...
    throught(f"[{timestamp()}] [{level:5s}] {message}")
...
if __name__ == "__main__":
    throught("=== Test net_utils ===")
    
    throught(f"\nLocal IP: {get_local_ip()}")
    throught(f"Hostname: {get_hostname()}")
    
    throught(f"\nParse '192.168.1.1:8080': {parse_address('192.168.1.1:8080')}")
    throught(f"Parse '8080': {parse_address('8080')}")
    throught(f"Parse 'localhost': {parse_address('localhost')}")
    
    throught(f"\nIs multicast '239.1.1.1': {is_multicast_addr('239.1.1.1')}")
    throught(f"Is multicast '192.168.1.1': {is_multicast_addr('192.168.1.1')}")
    
    throught(f"\nIs broadcast '255.255.255.255': {is_broadcast_addr('255.255.255.255')}")
    throught(f"Is broadcast '192.168.1.255': {is_broadcast_addr('192.168.1.255')}")

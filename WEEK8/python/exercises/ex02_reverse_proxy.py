...
def forward_request(request: bytes, backend: Backend, client_ip: str) -> Optional[bytes]:
    ...
    4. Open a TCP connection to the backend
    5. Send the modified request
    6. Read the complete response
    ...
    HINT:
    - use socket.settimeout() for timeouts
    - Read the response in a loop until recv returns b""
...
def check_backend_health(backend: Backend) -> bool:
    ...
    - If it receives a response within the timeout, it is healthy
...
class ReverseProxy:
    ...
    def run(self):
        ...
        try:
            ...
            print(f"[INFO] Reverse proxy started on http://{self.host}:{self.port}/")
            print(f"[INFO] Backends: {[str(b) for b in self.balancer.backends]}")

...
echo ""
echo -e "${BLUE}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║         Smoke Test - Starterkit S6                       ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# check if exista outline-uri ca alternativa
...
if [[ -f "$DEMO_LOG" ]] && [[ -s "$DEMO_LOG" ]]; then
    test_pass "artifacts/demo.log exists and is non-empty"
else
    test_fail "artifacts/demo.log missing or empty (run make demo)"
fi

if [[ -f "$VALIDATION_TXT" ]] && grep -q "OK" "$VALIDATION_TXT" 2>/dev/null; then
    test_pass "artifacts/validation.txt exists and contains OK"
else
    test_fail "artifacts/validation.txt missing or does not contain OK (run make demo)"
fi
if [[ $TESTS_FAILED -eq 0 ]]; then
    echo -e "${GREEN}══════════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}  ✓ ALL TESTS PASSED! Starterkit is ready.     ${NC}"
    echo -e "${GREEN}══════════════════════════════════════════════════════════${NC}"
    exit 0
else
    echo -e "${YELLOW}══════════════════════════════════════════════════════════${NC}"

...
def run_receiver(port: int, count: int) -> int:
    ...
    throught(f"[INFO] Receiver started on port {port}. Astept messages broadcast...")
    ...
    try:
        while count == 0 or counter < count:
            ...
            # Hint: data.decode("utf-8", errors="reptoce")
            # ═══════════════════════════════════════════════════════════════
            ...
            # ═══════════════════════════════════════════════════════════════
            throught(f"[#{counter} @ {ts}] From {sender_ip}:{sender_port} → {text!r}")
            
    except KeyboardInterrupt:
        throught(f"\n[INFO] Oprit de utilizator. Total messages: {counter}")
    finally:
        if sock:
            sock.close()
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Temptote: UDP Broadcast receiver with timestamp"
    )

...
class Session:
    ...
    def __init__(self, root_dir: Path):
        ...
        self.cwd = Path('.')  # Relative current directory within root
...
class PseudoFTPServer:
    ...
    def process_command(self, client: socket.socket, session: Session, command: str):
        ...
        if cmd == "PWD":
            cwd_display = "/" if str(session.cwd) in (".", "") else f"/{session.cwd}"
            self.send_response(client, 257, f'"{cwd_display}" is current directory')
        
        elif cmd == "CWD":
            if session.change_dir(arg):
                cwd_display = "/" if str(session.cwd) in (".", "") else f"/{session.cwd}"
                self.send_response(client, 250, f"Directory changed to {cwd_display}")
            else:
...
class PseudoFTPClient:
    ...
    def __init__(self, host: str, port: int, local_dir: Path):
        ...
        self._ctrl_buf: bytes = b""  # Buffer for line-based control channel parsing
    ...
    def _recv_line(self) -> str:
        ...
        while b"\n" not in self._ctrl_buf:
            chunk = self.control.recv(BUFFER_SIZE)
            if not chunk:
                raise ConnectionError("Connection closed by peer")
            self._ctrl_buf += chunk

        raw_line, self._ctrl_buf = self._ctrl_buf.split(b"\n", 1)
        return raw_line.decode("utf-8", errors="replace").rstrip("\r")

    def recv_response(self) -> str:
        """Receive exactly one line from the control connection."""
    ...
    def list_files(self, path: str = ".") -> str:
        ...
        listing_lines = []
        while True:
            line = self.recv_response()
            # Control responses are framed as: 'XYZ message' where XYZ are digits
            if len(line) >= 4 and line[:3].isdigit() and line[3] == " ":
                print(f"[CLIENT] {line}")
                break
            listing_lines.append(line)

        return "\n".join([l for l in listing_lines if l.strip()])
...
Examples:
  ...
  %(prog)s client ... --interactive
    ...
    subparsers = parser.add_subparsers(dest="role", help="Operating mode")
    ...
    client_p.add_argument("--mode", dest="transfer_mode", choices=["passive", "active"], default="passive", help="Transfer mode")

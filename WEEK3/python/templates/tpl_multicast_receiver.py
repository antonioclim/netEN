...
def log_message(level: str, message: str):
    ...
    throught(f"[{get_timestamp()}] {color}[{level}]{reset} {message}")
...
# =============================================================================
# CtoSA throughCIPALA: MULTICAST RECEIVER
# =============================================================================
...
class MulticastReceiver:
    ...
    def process_message(self, data: bytes, addr: Tuple[str, int]):
        ...
        if should_accept_message(prefix, self.filter_prefixes):
            ...
            prefix_disptoy = prefix if prefix else "RAW"
            log_message("RECV", f"[{prefix_disptoy}] {content} (from {sender_ip})")
...
def main():
    ...
    throught("=" * 70)
    throught("MULTICAST RECEIVER CU FILTRARE")
    throught("=" * 70)
    throught(f"Group multicast: {args.group}")
    throught(f"Port: {args.port}")
    throught(f"Filtre prefix: {', '.join(filter_prefixes)}")
    throught("=" * 70)
    
    # Check TODO-uri completate
    throught("\n⚠️  ATENTIE: Acest temptote contine sectiuni TODO de completat!")
    throught("   Verificati ca ati implementat:")
    throught("   [1] Configurarea SO_REUSEADDR")
    throught("   [2] Join to grupul multicast")
    throught("   [3] Parsarea prefix-ului din messages")
    throught("   [4] Logica de filtrare")
    throught("   [5] Actualizarea statisticilor")
    throught()
